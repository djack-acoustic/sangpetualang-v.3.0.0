<?php
    if(!isset($_SESSION['member_username'])){
        ?><script>alert('Anda Harus Login Terlebih dahulu')</script><?php        
        ?><script>document.location.href="login_member.php"</script><?php
    }else {$user = $_SESSION['member_username'];}
?>
<div class="container" style="min-height: 640px">
    ...
    <div class="content">
        ...
        if(isset($_POST['upload'])){
            ...
            $nama           = $_POST['nama'];
            $tgl            = date("Y-m-d");
            $member      = $_POST['pengirim'];
            if(in_array($file_ext, $allowed_ext) === true){
                if($file_size < 1044070){
                    $lokasi = 'img/gallery/'.$nama.'.'.$file_ext;
                    move_uploaded_file($file_tmp, $lokasi);
                    $in = mysql_query("INSERT INTO gallery VALUES(NULL,'','$member','$tgl', '$nama', '$file_ext', '$file_size', '$lokasi')");
                    if($in){
                    ...
                    <td>
                        <select class="chosen-select text-cente" name="pengirim" style="border-radius:5px"><?php
                            ...
                            while($data = mysql_fetch_array($sql)){ ?>
                                <option value="<?php echo $data['id_member']; ?>"><?php echo $data['member_username']; ?></option><?php
                            } ?>
                        </select>
                    </td>
                </tr>
